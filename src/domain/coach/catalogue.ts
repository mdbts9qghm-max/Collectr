...
/**
 * Der Einheitenkatalog. Geplant wird Laufen.
 *
 * > Das gesamte Ausdauervolumen entsteht durch Laufen. Rudergerät und
 * > Crosstrainer sind keine zulässigen Trainingsformen. Das Rad wird nie
 * > geplant — es steht ausschließlich als Abstufungsstufe zur Verfügung, wenn
 * > die Erholung einen lockeren Lauf nicht mehr trägt.
 *
 * Der Unterschied ist der ganze Punkt. **Geplant** wird nur Laufen: jede
 * Volumenvorgabe, jedes Phasenziel, jede Schlüsseleinheit ist ein Lauf. Das Rad
 * taucht nur auf, wenn der Tag nach unten geht, und zwar an genau einer Stelle
 * der Kette — zwischen lockerem Lauf und Gehen. Es ist die gelenkschonende
 * Stufe, auf der ein aerober Reiz noch möglich ist, wenn die Beine keinen Stoß
 * mehr vertragen.
 *
 * > Gehen bleibt die letzte Stufe vor Ruhe.
 */

export const HARTE_REGEL_LAUFEN =
  'Das gesamte Ausdauervolumen entsteht durch Laufen. Rudergerät und Crosstrainer sind keine zulässigen Trainingsformen. Das Rad wird nie geplant — es steht nur als Abstufungsstufe zur Verfügung, wenn die Erholung einen lockeren Lauf nicht mehr trägt. Gehen bleibt die letzte Stufe vor Ruhe.';
...
export type Discipline = 'lauf' | 'rad' | 'kraft' | 'ruhe';
...
const ENTRIES: CatalogueEntry[] = [
  ...
  },
  {
    kind: 'rad',
    label: 'Rad',
    headline: 'Heute auf dem Rad, ohne Stoß.',
    discipline: 'rad',
    zone: 2,
    // Eine Abstufungsstufe, keine Planungsoption: dieser Eintrag wird nie
    // ausgewählt, er wird nur erreicht.
    load: 22,
    minRecovery: 25,
    defaultMinutes: 45,
    minMinutes: 25,
 ...
 *
 * Eine Kette geht immer nach unten und endet auf `ruhe`. Das Rad steht in jeder
 * Laufkette an genau einer Stelle: unter dem lockeren Lauf, über dem Gehen. Es
 * ist damit erreichbar, aber nie wählbar — der Planer greift auf diese Ketten
 * nur zu, wenn er abstuft.
 *
 * Warum das Rad dort und nicht woanders: an dieser Stelle ist der Grund für die
 * Abstufung fast immer muskuloskelettal, nicht kardial. Wer keinen Stoß mehr
 * verträgt, kann meist noch treten — und hält damit den aeroben Reiz, statt ihn
 * ganz zu verlieren. Ist auch das zu viel, bleibt Gehen, dann Ruhe.
 */
const CHAINS: Record<SessionKind, SessionKind[]> = {
  intervall: ['intervall', 'intervall_kurz', 'lockerer_lauf', 'rad', 'gehen', 'ruhe'],
  intervall_kurz: ['intervall_kurz', 'lockerer_lauf', 'rad', 'gehen', 'ruhe'],
  longrun: ['longrun', 'longrun_verkuerzt', 'lockerer_lauf', 'rad', 'gehen', 'ruhe'],
  longrun_verkuerzt: ['longrun_verkuerzt', 'lockerer_lauf', 'rad', 'gehen', 'ruhe'],
  grundlagenlauf: ['grundlagenlauf', 'lockerer_lauf', 'rad', 'gehen', 'ruhe'],
  lockerer_lauf: ['lockerer_lauf', 'rad', 'gehen', 'ruhe'],
  rad: ['rad', 'gehen', 'ruhe'],
  gehen: ['gehen', 'ruhe'],
  ruhe: ['ruhe'],
