...
import { DELOAD_CUT, MAX_GROWTH, phaseForWeek } from './phases.ts';
...
export function strengthTargetFor(input: {
  ...
  const deloadShare = input.isDeload ? 1 : 0;

  const withDeload = (minutes: number) => Math.round(minutes * (1 - DELOAD_CUT * deloadShare));
...
export interface StrengthInput {
  ...
  /**
   * Trägt dieser Tag des Schichtrhythmus überhaupt schwere Beinlast?
   *
   * Am Nachtschichttag nicht: danach stehen zwölf Stunden Dienst an, und
   * schwere Beine sind dort ein Sicherheitsthema, kein Trainingsthema.
   */
  heavyLegsAllowedToday?: boolean;
}
...
export function planStrength(input: StrengthInput): StrengthPlan {
  ...
  if (input.heavyLegsAllowedToday === false) {
    legsAllowed = false;
    add('Dieser Schichttag trägt keine schweren Beine', 0);
  }
